...
import {
   ...
   Send, Loader2, Image, Trash2, Edit,
   X, Heart, MessageCircle, Search, Award
} from 'lucide-react';
...
const AlumniCommunity = () => {
   ...
   }, [user, categoryFilter, searchQuery, selectedPost]);
   ...
   const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
         if (file.size > 5 * 1024 * 1024) {
            showToast({ title: 'File Too Large', message: 'Maximum size is 5MB.', type: 'error' });
            return;
         }
         setImageFile(file);
   ...
   const handleSubmitPost = async (e: React.FormEvent) => {
      ...
      try {
         ...
         fetchPosts();
         showToast({ title: 'Success', message: 'Post shared!', type: 'success' });
      } catch (error: any) {
   ...
   const handleEditPost = async (e: React.FormEvent) => {
      ...
      try {
         ...
         setEditingPost(null);
         fetchPosts();
         showToast({ title: 'Success', message: 'Post updated!', type: 'success' });
      } catch (error: any) {
   ...
   const filteredPosts = posts; // Now handled server-side in fetchPosts
               ...
               <div className="lg:col-span-2 space-y-6">
                     ...
                     <form onSubmit={handleSubmitPost} className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-2xl space-y-5">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-4">
                           <div className="flex items-center gap-4">
                              <label className="flex items-center gap-2 text-slate-400 hover:text-blue-600 text-sm font-bold transition-colors cursor-pointer">
                                 <Image className="w-5 h-5" /> Add Image
                                 <input type="file" className="hidden" accept="image/*" onChange={handleImageChange} />
                              </label>
                              {imagePreview && (
                                 <div className="relative w-12 h-12">
                                    <img src={imagePreview} className="w-full h-full object-cover rounded-lg" alt="" />
                                    <button type="button" onClick={() => { setImageFile(null); setImagePreview(null); }} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-0.5">
                                       <X className="w-3 h-3" />
                                    </button>
                                 </div>
                              )}
                  ...
                  {editingPost && (
                     <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                        <form onSubmit={handleEditPost} className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-2xl w-full max-w-xl space-y-5 animate-in zoom-in-95">
                           <div className="flex justify-between items-center">
                              <h3 className="font-black text-slate-900 text-lg tracking-tight">Edit Post</h3>
                              <button type="button" onClick={() => setEditingPost(null)} className="p-2 bg-slate-100 rounded-full text-slate-400 hover:text-slate-600 hover:rotate-90 transition-all">
                                 <X className="w-4 h-4" />
                              </button>
                           </div>
                           ...
                           <div className="flex items-center justify-end">
                              <button
                                 type="submit"
                                 disabled={posting || !editingPost.content.trim()}
                                 className="px-8 py-3 bg-blue-600 text-white rounded-2xl font-black hover:bg-blue-700 shadow-lg"
                              >
                  ...
                  {loading ? (
                     ...
                     filteredPosts.map(post => (
                        <div key={post.id} className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500">
                           ...
                           <div className="flex items-center gap-2 mb-3">
                              {post.updated_at && (
                                 <span className="text-[10px] font-black uppercase text-blue-500 bg-blue-50 px-2 py-0.5 rounded-full tracking-tighter">Edited</span>
                              )}
                           </div>
