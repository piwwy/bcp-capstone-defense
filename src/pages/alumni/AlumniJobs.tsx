...
import { useJobs, useMyApplications, useAppliedJobIds, useSavedJobIds } from '../../hooks/useSupabaseQuery';
...
const AlumniJobs: React.FC = () => {
  ...
  const { data: savedJobIds = new Set<string>(), } = useSavedJobIds(user?.id);
  ...
  const toggleSaveJob = async (jobId: string) => {
    ...
    try {
      if (isSaved) {
        ...
        await supabase
          .from('alumni_saved_jobs')
          .insert([{ alumni_id: user.id, job_id: jobId }]);
      }

      queryClient.invalidateQueries({ queryKey: ['saved_job_ids', user.id] });
      showToast({
      ...
      showToast({ title: 'Error', message: 'Failed to update saved jobs.', type: 'error' });
    }
  };
  ...
  return (
    <PageTransition>
      <div className="max-w-7xl mx-auto space-y-6 pb-10 transition-colors">
        ...
        <div className="relative h-[240px] rounded-[2.5rem] bg-gradient-to-r from-slate-900 via-blue-900 to-indigo-900 overflow-hidden shadow-2xl flex items-end px-10 pb-8">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between w-full gap-4">
            <div>
              ...
              <p className="text-blue-200 text-sm font-medium max-w-md">Exclusive career opportunities for BCP Alumni graduates.</p>
            </div>
            <div className="flex bg-white/10 backdrop-blur-sm border border-white/20 p-1.5 rounded-2xl w-full md:w-auto">
              <button
                onClick={() => { setMainView('discover'); setShowDetail(false); }}
                className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${mainView === 'discover' ? 'bg-white text-blue-600 shadow-lg' : 'text-white/60 hover:text-white dark:text-gray-400 dark:hover:text-white'}`}
              >
                <Briefcase className="w-4 h-4" /> Find Work
              </button>
              <button
                onClick={() => { setMainView('my-apps'); setShowDetail(false); }}
                className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${mainView === 'my-apps' ? 'bg-white text-blue-600 shadow-lg' : 'text-white/60 hover:text-white dark:text-gray-400 dark:hover:text-white'}`}
              >
            ...
            <div className={`${showDetail ? 'w-[45%]' : 'w-full'} transition-all duration-500 space-y-4`}>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3.5 bg-white dark:bg-dark-800 rounded-2xl border border-slate-100 dark:border-gray-700 text-sm font-medium focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-900 outline-none shadow-sm dark:text-white dark:placeholder-gray-500"
                  value={searchTerm}
              ...
              <div className="flex items-center gap-1 bg-white/80 dark:bg-dark-800/80 backdrop-blur p-1.5 rounded-2xl border border-slate-100 dark:border-gray-700 shadow-sm">
                {FEED_TABS.map(tab => (
                  <button
                    key={tab.id}
                    onClick={() => setFeedTab(tab.id)}
                    className={`px-4 py-2.5 rounded-xl text-xs font-black transition-all ${feedTab === tab.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-200 dark:shadow-blue-900/40' : 'text-slate-400 dark:text-gray-500 hover:text-slate-600 dark:hover:text-gray-300 hover:bg-slate-50 dark:hover:bg-gray-700'}`}
                  >
                    {tab.label}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-1.5 px-1">
                {COURSES.slice(0, showDetail ? 6 : 14).map(c => (
                  <button
                    key={c.value}
                    onClick={() => setSelectedDept(c.value)}
                    className={`px-3 py-1.5 rounded-full text-[10px] font-bold transition-all ${selectedDept === c.value ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-50 dark:bg-gray-700 text-slate-500 dark:text-gray-400 hover:bg-slate-100 dark:hover:bg-gray-600 border border-slate-100 dark:border-gray-600'}`}
                  >
                ...
                {loading ? (
                  ...
                  return (
                    <div
                      ...
                      className={`group bg-white dark:bg-dark-800 p-5 rounded-2xl border cursor-pointer transition-all duration-300 hover:shadow-lg dark:hover:shadow-blue-900/10 hover:-translate-y-0.5 ${isSelected ? 'border-blue-500 bg-blue-50/30 dark:bg-blue-900/20 shadow-md' : 'border-slate-100 dark:border-gray-700 hover:border-blue-200 dark:hover:border-blue-900/50'}`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <p className="text-[10px] text-slate-400 dark:text-gray-500 font-medium mb-1">Posted {timeAgo(job.created_at)}</p>
                          <h3 className="text-base font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-tight mb-1">{job.title}</h3>
                          <p className="text-xs text-slate-500 dark:text-gray-400 mb-2">{job.company}</p>
                          <p className="text-xs text-slate-400 dark:text-gray-500 line-clamp-2 mb-3">{job.description}</p>

                          {/* Tags */}
                          <div className="flex flex-wrap gap-1.5 mb-3">
                            {job.work_type && <span className="px-2 py-0.5 bg-slate-50 dark:bg-gray-700 border border-slate-100 dark:border-gray-600 rounded-full text-[10px] font-bold text-slate-500 dark:text-gray-400">{job.work_type}</span>}
                            {job.salary_range && <span className="px-2 py-0.5 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-full text-[10px] font-bold text-emerald-600 dark:text-emerald-400">₱{job.salary_range}</span>}
                            {job.location && <span className="px-2 py-0.5 bg-slate-50 dark:bg-gray-700 border border-slate-100 dark:border-gray-600 rounded-full text-[10px] font-bold text-slate-500 dark:text-gray-400 flex items-center gap-1"><MapPin className="w-2.5 h-2.5" />{job.location}</span>}
                          </div>

                          {/* Course tags */}
                          {job.target_courses && job.target_courses.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                              {job.target_courses.slice(0, 4).map((c: string) => (
                                <span key={c} className={`px-2 py-0.5 rounded text-[9px] font-bold ${c === userCourse ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' : 'bg-slate-50 dark:bg-gray-700 text-slate-400 dark:text-gray-500'}`}>{c}</span>
                              ))}
                              {job.target_courses.length > 4 && <span className="text-[9px] text-slate-400 dark:text-gray-500 font-bold">+{job.target_courses.length - 4}</span>}
                            </div>
                          )}
                        </div>

                        <div className="flex flex-col items-center gap-2 flex-shrink-0">
                          <button onClick={e => { e.stopPropagation(); toggleSaveJob(job.id); }} className={`p-2 rounded-full transition-all active:scale-90 ${isSaved ? 'text-red-500 bg-red-50 dark:bg-red-900/20' : 'text-slate-300 dark:text-gray-600 hover:text-red-400 dark:hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10'}`}>
            ...
            {loading ? (
              ...
              <div key={app.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 space-y-4">
                ...
                <div className="flex justify-end">
                  <button onClick={() => handleWithdraw(app.id)} className="text-[10px] font-bold text-slate-400 uppercase hover:text-rose-500 transition-all">
                    Withdraw
                  </button>
                </div>
              </div>
