  ...
  return (
    <PageTransition>
      <div className="max-w-7xl mx-auto space-y-10 pb-20">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-gray-700 pb-4 transition-colors">
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setActiveCategory(cat)}
                className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeCategory === cat ? 'bg-slate-900 dark:bg-dark-900 text-white shadow-lg' : 'text-slate-500 dark:text-gray-400 hover:bg-slate-100 dark:hover:bg-gray-800'
                  }`}
              >
                {cat}
              </button>
