...
const AlumniMessages = () => {
  ...
  const handleFileUpload = async (file: File): Promise<{ url: string; name: string; type: string } | null> => {
    try {
      ...
      showToast({ title: 'Upload Failed', message: err.message || 'Could not upload file.', type: 'error' });
      return null;
    }
  };
  ...
  const sendMessage = async () => {
    ...
    if (selectedFile) {
      ...
      setUploading(false);
      if (!attachment) return; // Stop if upload failed
                ...
                {filteredConversations.length === 0 ? (
                  ...
                  filteredConversations.map(conv => (
                    <div
                      ...
                      className={`flex items-center gap-3 px-4 py-4 cursor-pointer transition-all border-b border-slate-50 group ${selectedPartner === conv.partnerId ? 'bg-blue-50 border-l-4 border-l-blue-500' : 'hover:bg-slate-50'
                        }`}
                    >
                          ...
                          {group.messages.map(msg => {
                            ...
                            return (
                              <div key={msg.id} className={`flex mb-2 ${isMine ? 'justify-end' : 'justify-start'}`}>
                                ...
                                <div className={`max-w-[70%] px-4 py-2.5 ${isMine
