...
const PendingApproval: React.FC = () => {
  ...
  // Get name/email from navigation state if available (from Register page)
  const meta = location.state as { name?: string; email?: string } || {};
  ...
  const displayName = meta.name || user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'Alumni';
  const userImage = user?.user_metadata?.avatar_url || `https://ui-avatars.com/api/?name=${displayName}&background=0D8ABC&color=fff`;
